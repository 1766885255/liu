(function($) {	
	...
	$.fn.cardFormat = function(pattern) {
		$(this).each(function(){
			...
			var num = JH.NumberFormat.format(that.val(),pattern).replace(/,/g,' ');
			if(num == 0){
				that.before('<div class="num">&nbsp;</div>');
			}else{
				that.before('<div class="num">'+num+'</div>');
			}
			that.prev().css({top:-that.outerHeight()-3,width:that.outerWidth()-2});
	...
	$.fn.box = function(options) {
		...
		$(this).each(function(){
			...
			if(!that.parentsUntil('.boxwrap').parent().hasClass('boxwrap')){
				var wrapBox = $('<div class="boxwrap"><div class="boxbg"></div><div class="box"><b class="close"></b><div class="title">'+sets.title+'</div><div class="content"></div></div></div>');
				...
				wrapBox.find('.content').css({width:sets.width-30,height:sets.height-90,margin:15});
				$('body').append(wrapBox);
				that.show();
